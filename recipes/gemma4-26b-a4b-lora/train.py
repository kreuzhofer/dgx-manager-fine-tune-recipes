...
Gemma 4 E4B specific: use_cache must remain True during training.
Setting use_cache=False corrupts attention computation and produces
garbage logits (known Gemma 4 bug). We patch gradient_checkpointing_enable
to preserve use_cache=True.
...
def format_example(example, tokenizer, max_seq_length):
    ...
    if "conversations" in example:
        ...
        # QA format (e.g., b-mc2/sql-create-context)
...
def main():
    ...
    # Gemma 4 bug fix: use_cache=False corrupts attention outputs.
    # gradient_checkpointing_enable sets use_cache=False by default.
    # Force it back to True to avoid garbage logits.
    ...
    if args.dataset.endswith(".jsonl") or args.dataset.endswith(".json"):
        ...
        # HuggingFace dataset ID (e.g., "b-mc2/sql-create-context")
